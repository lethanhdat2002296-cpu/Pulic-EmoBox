      ...
      <div style="max-width:680px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden">
        ...
        <div style="padding:24px">
          ...
          <div style="margin-top:16px;border-top:1px solid #eee;padding-top:16px">
            ...
            ${Number(order.discount || 0) > 0 ? `<p style="margin:6px 0">Voucher ${escapeHtml(order.voucher && order.voucher.code || '')}: <strong>-${money(order.discount)}</strong></p>` : ''}
...
function orderText(payload, audience) {
  ...
  const lines = [
    ...
    Number(order.discount || 0) > 0 ? `Voucher ${order.voucher && order.voucher.code || ''}: -${money(order.discount)}` : '',
...
function giftScheduleText(payload) {
  ...
  return [
    ...
    `Mã lịch: ${event.id || payload.localEventId || ''}`,
...
function giftScheduleHtml(payload) {
  ...
  const content = `
    ...
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Mã lịch: <strong>${escapeHtml(event.id || payload.localEventId || '')}</strong></p>
...
function contactHtml(payload) {
  const content = `
    <p style="margin:0 0 16px">EmoBox vừa nhận được một lời nhắn mới từ website.</p>
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Họ tên: <strong>${escapeHtml(payload.name)}</strong></p>
      <p style="margin:6px 0">Email: <strong>${escapeHtml(payload.email)}</strong></p>
      <p style="margin:6px 0">Chủ đề: <strong>${escapeHtml(payload.subject)}</strong></p>
    </div>
    <p style="white-space:pre-wrap;line-height:1.6">${escapeHtml(payload.message)}</p>
  `;
  return emailShell('Tin nhắn liên hệ mới', content);
}
...
async function sendContactEmail(payload) {
  ...
  return sendMessages([{
    ...
    subject: `EmoBox liên hệ mới: ${payload.subject || 'Website'}`,
    text: [
...
function orderStatusText(payload) {
  const order = payload.order || {};
  return [
    payload.title || 'Cập nhật trạng thái đơn hàng EmoBox',
    `Mã đơn hàng: ${payload.orderCode || order.orderCode || ''}`,
    `Trạng thái thanh toán: ${payload.paymentStatus || order.paymentStatus || ''}`,
    `Trạng thái đơn: ${payload.orderStatus || order.orderStatus || ''}`,
    `Tổng cộng: ${money(order.total || order.totalAmount)}`,
    payload.note || ''
  ].filter(Boolean).join('\n');
}

function orderStatusHtml(payload) {
  const order = payload.order || {};
  const content = `
    <p style="margin:0 0 16px">${escapeHtml(payload.message || 'Đơn hàng của bạn vừa được cập nhật trạng thái.')}</p>
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Mã đơn hàng: <strong>${escapeHtml(payload.orderCode || order.orderCode || '')}</strong></p>
      <p style="margin:6px 0">Trạng thái thanh toán: <strong>${escapeHtml(payload.paymentStatus || order.paymentStatus || '')}</strong></p>
      <p style="margin:6px 0">Trạng thái đơn: <strong>${escapeHtml(payload.orderStatus || order.orderStatus || '')}</strong></p>
      <p style="margin:6px 0">Tổng cộng: <strong>${money(order.total || order.totalAmount)}</strong></p>
    </div>
    ${payload.note ? `<p style="margin:0;color:#6b625c">${escapeHtml(payload.note)}</p>` : ''}
  `;
  return emailShell(payload.title || 'Cập nhật đơn hàng EmoBox', content);
}
...
async function sendOrderStatusEmail(payload) {
  ...
  const config = getConfig();
  const to = payload.to || (payload.contact && payload.contact.email) || (payload.user && payload.user.email);
  return sendMessages([{
    from: config.from,
    to,
    subject: payload.subject || `EmoBox cập nhật đơn hàng ${payload.orderCode || ''}`,
    text: orderStatusText(payload),
    html: orderStatusHtml(payload)
  }]);
}

function scheduleStatusHtml(payload) {
  const event = payload.event || {};
  const content = `
    <p style="margin:0 0 16px">${escapeHtml(payload.message || 'Lịch tặng quà của bạn vừa được cập nhật.')}</p>
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Mã lịch: <strong>${escapeHtml(event.id || payload.localEventId || '')}</strong></p>
      <p style="margin:6px 0">Người nhận: <strong>${escapeHtml(event.recipient || '')}</strong></p>
      <p style="margin:6px 0">Ngày tặng: <strong>${escapeHtml(dateLabel(event.date))}</strong></p>
      <p style="margin:6px 0">Gói quà: <strong>${escapeHtml(event.pkgName || 'Quà tặng')}</strong></p>
      ${payload.refundedAmount ? `<p style="margin:6px 0">Đã hoàn ví: <strong>${money(payload.refundedAmount)}</strong></p>` : ''}
    </div>
    ${payload.note ? `<p style="margin:0;color:#6b625c">${escapeHtml(payload.note)}</p>` : ''}
  `;
  return emailShell(payload.title || 'Cập nhật lịch tặng quà EmoBox', content);
}

async function sendGiftScheduleStatusEmail(payload) {
  if (!isEmailConfigured()) {
    return { sent: false, skipped: true, reason: 'Chưa cấu hình email' };
  }
  ...
  const config = getConfig();
  const event = payload.event || {};
  const to = payload.to || event.email || (payload.user && payload.user.email);
  return sendMessages([{
    from: config.from,
    to,
    subject: payload.subject || `EmoBox cập nhật lịch tặng quà ${event.id || payload.localEventId || ''}`,
    text: [
      payload.title || 'Cập nhật lịch tặng quà EmoBox',
      `Mã lịch: ${event.id || payload.localEventId || ''}`,
      `Người nhận: ${event.recipient || ''}`,
      `Ngày tặng: ${dateLabel(event.date)}`,
      payload.refundedAmount ? `Đã hoàn ví: ${money(payload.refundedAmount)}` : '',
      payload.note || ''
    ].filter(Boolean).join('\n'),
    html: scheduleStatusHtml(payload)
  }]);
}
...
function voucherHtml(payload) {
  const voucher = payload.voucher || {};
  const content = `
    <p style="margin:0 0 16px">EmoBox gui ban ma uu dai moi.</p>
    <div style="border:1px dashed #0284c7;border-radius:12px;padding:18px;margin:16px 0;text-align:center;background:#f0f9ff">
      <p style="margin:0 0 8px;color:#075985;font-size:13px;font-weight:700">MA VOUCHER</p>
      <p style="margin:0;font-size:30px;font-weight:800;letter-spacing:2px;color:#0f172a">${escapeHtml(voucher.code)}</p>
      <p style="margin:10px 0 0;font-size:18px;color:#0284c7;font-weight:800">Giam ${Number(voucher.discountPercent || voucher.discount_percent || 0)}%</p>
    </div>
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Ap dung cho: <strong>${escapeHtml(voucherCustomerLabel(voucher.customerType || voucher.customer_type))}</strong></p>
      <p style="margin:6px 0">Han su dung: <strong>${escapeHtml(dateLabel(voucher.expiresAt || voucher.expires_at))}</strong></p>
    </div>
    <p style="margin:0;color:#6b625c">Nhap ma nay tai buoc thanh toan de he thong tu dong kiem tra va tru tien phu hop.</p>
  `;
  return emailShell('Voucher uu dai EmoBox', content);
}

async function sendVoucherEmail(payload) {
  if (!isEmailConfigured()) {
    return { sent: false, skipped: true, reason: 'Chua cau hinh email' };
  }
  ...
  return sendMessages(recipients.map(to => ({
    from: config.from,
    to,
    subject: `EmoBox gui voucher ${payload.voucher && payload.voucher.code || ''}`,
    text: [
      'EmoBox gui ban ma uu dai moi',
      `Ma voucher: ${payload.voucher && payload.voucher.code || ''}`,
      `Giam: ${Number(payload.voucher && (payload.voucher.discountPercent || payload.voucher.discount_percent) || 0)}%`,
      `Ap dung cho: ${voucherCustomerLabel(payload.voucher && (payload.voucher.customerType || payload.voucher.customer_type))}`,
      `Han su dung: ${dateLabel(payload.voucher && (payload.voucher.expiresAt || payload.voucher.expires_at))}`
    ].join('\n'),
    html: voucherHtml(payload)
  })));
}

function customGiftText(payload, mode) {
  const request = payload.request || {};
  const customer = request.customer || {};
  const title = mode === 'quote'
    ? 'Yêu cầu thiết kế quà tặng của bạn đã được báo giá'
    : 'EmoBox đã nhận yêu cầu thiết kế quà tặng của bạn';
  return [
    title,
    `Mã yêu cầu: ${request.requestCode || request.request_code || ''}`,
    `Khách hàng: ${customer.name || request.customerName || ''}`,
    `Gói quà: ${request.giftPackage || request.gift_package || ''}`,
    `Loại quà: ${request.giftType || request.gift_type || ''}`,
    mode === 'quote' ? `Giá báo: ${money(request.quotedAmount || request.quoted_amount)}` : 'Trạng thái: Chờ EmoBox báo giá',
    payload.quoteLink ? `Link kiểm tra/xác nhận: ${payload.quoteLink}` : '',
    '',
    mode === 'quote'
      ? 'Vui lòng mở link trên để xác nhận giá, hủy yêu cầu hoặc yêu cầu báo giá lại.'
      : 'EmoBox sẽ kiểm tra yêu cầu và gửi báo giá cho bạn trong thời gian sớm nhất.'
  ].filter(Boolean).join('\n');
}

function customGiftHtml(payload, mode) {
  const request = payload.request || {};
  const customer = request.customer || {};
  const isQuote = mode === 'quote';
  const content = `
    <p style="margin:0 0 16px">Xin chào <strong>${escapeHtml(customer.name || request.customerName || 'bạn')}</strong>, ${isQuote
      ? 'EmoBox đã hoàn tất báo giá cho yêu cầu thiết kế quà tặng của bạn.'
      : 'EmoBox đã nhận được yêu cầu thiết kế quà tặng của bạn và sẽ báo giá sớm nhất có thể.'}</p>
    <div style="border-top:1px solid #eee;border-bottom:1px solid #eee;padding:16px 0;margin:16px 0">
      <p style="margin:6px 0">Mã yêu cầu: <strong>${escapeHtml(request.requestCode || request.request_code || '')}</strong></p>
      <p style="margin:6px 0">Gói quà: <strong>${escapeHtml(request.giftPackage || request.gift_package || '')}</strong></p>
      <p style="margin:6px 0">Loại quà: <strong>${escapeHtml(request.giftType || request.gift_type || '')}</strong></p>
      ${isQuote ? `<p style="margin:6px 0;font-size:18px">Giá báo: <strong>${money(request.quotedAmount || request.quoted_amount)}</strong></p>` : ''}
      ${request.quoteNote || request.quote_note ? `<p style="margin:6px 0">Ghi chú báo giá: ${escapeHtml(request.quoteNote || request.quote_note)}</p>` : ''}
    </div>
    ${payload.quoteLink ? `<p style="margin:18px 0"><a href="${escapeHtml(payload.quoteLink)}" style="background:#0284c7;color:#fff;text-decoration:none;border-radius:8px;padding:12px 16px;font-weight:700;display:inline-block">${isQuote ? 'Kiểm tra và xác nhận giá' : 'Theo dõi yêu cầu'}</a></p>` : ''}
    <p style="margin:0;color:#6b625c">${isQuote
      ? 'Nếu giá chưa phù hợp, bạn có thể yêu cầu EmoBox báo giá lại ngay trong trang kiểm tra.'
      : 'Cảm ơn bạn đã tin tưởng EmoBox để tạo một món quà thật riêng.'}</p>
  `;
  return emailShell(isQuote ? 'Báo giá thiết kế quà tặng EmoBox' : 'Đã nhận yêu cầu thiết kế quà tặng', content);
}

async function sendCustomGiftRequestEmail(payload) {
  if (!isEmailConfigured()) {
    return { sent: false, skipped: true, reason: 'Chua cau hinh email' };
  }
  ...
  const request = payload.request || {};
  const customer = request.customer || {};
  const customerTo = customer.email || request.customerEmail || request.email;
  const messages = [];
  ...
  if (customerTo) {
    messages.push({
      from: config.from,
      to: customerTo,
      subject: `EmoBox đã nhận yêu cầu thiết kế quà ${request.requestCode || request.request_code || ''}`,
      text: customGiftText(payload, 'request'),
      html: customGiftHtml(payload, 'request')
    });
  }
  ...
  if (config.adminTo && !sameRecipient) {
    messages.push({
      ...
      to: config.adminTo,
      replyTo: customerTo || undefined,
      subject: `Yêu cầu thiết kế quà mới ${request.requestCode || request.request_code || ''}`,
      text: customGiftText(payload, 'request'),
      html: customGiftHtml(payload, 'request')
    });
  }
...
async function sendCustomGiftQuoteEmail(payload) {
  if (!isEmailConfigured()) {
    return { sent: false, skipped: true, reason: 'Chua cau hinh email' };
  }
  ...
  return sendMessages([{
    from: config.from,
    to: customerTo,
    subject: `EmoBox đã báo giá yêu cầu ${request.requestCode || request.request_code || ''}`,
    text: customGiftText(payload, 'quote'),
    html: customGiftHtml(payload, 'quote')
  }]);
}
