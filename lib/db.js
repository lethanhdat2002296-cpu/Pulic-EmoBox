...
CREATE TABLE IF NOT EXISTS "B30Vouchers" (
  voucher_id BIGSERIAL PRIMARY KEY,
  code TEXT NOT NULL UNIQUE,
  discount_percent INTEGER NOT NULL CHECK (discount_percent IN (5, 10, 15, 20)),
  customer_type TEXT NOT NULL DEFAULT 'all',
  expires_at TIMESTAMPTZ NOT NULL,
  send_method TEXT NOT NULL DEFAULT 'email_all',
...
CREATE TABLE IF NOT EXISTS "B30Orders" (
  ...
  voucher_id BIGINT REFERENCES "B30Vouchers"(voucher_id) ON DELETE SET NULL,
  voucher_code TEXT,
...
CREATE TABLE IF NOT EXISTS "B30VoucherRedemptions" (
  redemption_id BIGSERIAL PRIMARY KEY,
  voucher_id BIGINT NOT NULL REFERENCES "B30Vouchers"(voucher_id) ON DELETE CASCADE,
  user_id BIGINT REFERENCES "B20Users"(user_id) ON DELETE SET NULL,
  order_id BIGINT REFERENCES "B30Orders"(order_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS "B30CustomGiftRequests" (
...
CREATE TABLE IF NOT EXISTS "B20CustomGiftContacts" (
  contact_id BIGSERIAL PRIMARY KEY,
  request_id BIGINT NOT NULL UNIQUE REFERENCES "B30CustomGiftRequests"(request_id) ON DELETE CASCADE,
  user_id BIGINT REFERENCES "B20Users"(user_id) ON DELETE SET NULL,
  full_name TEXT NOT NULL,
  email TEXT NOT NULL,
  phone TEXT,
...
CREATE TABLE IF NOT EXISTS "B30CustomGiftRequestEvents" (
  event_id BIGSERIAL PRIMARY KEY,
  request_id BIGINT NOT NULL REFERENCES "B30CustomGiftRequests"(request_id) ON DELETE CASCADE,
  event_type TEXT NOT NULL,
  status TEXT,
  actor_type TEXT,
  actor_id TEXT,
...
ALTER TABLE IF EXISTS "B30Orders"
  ...
  ADD COLUMN IF NOT EXISTS voucher_id BIGINT REFERENCES "B30Vouchers"(voucher_id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS "IX_B30Vouchers_code"
  ON "B30Vouchers" (UPPER(code));

CREATE INDEX IF NOT EXISTS "IX_B30VoucherRedemptions_user_id"
  ON "B30VoucherRedemptions" (user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS "IX_B30CustomGiftRequests_user_id"
  ON "B30CustomGiftRequests" (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS "IX_B30CustomGiftRequests_status"
  ON "B30CustomGiftRequests" (status, created_at DESC);

CREATE INDEX IF NOT EXISTS "IX_B20CustomGiftContacts_email"
  ON "B20CustomGiftContacts" (LOWER(email));

CREATE INDEX IF NOT EXISTS "IX_B30CustomGiftRequestEvents_request"
  ON "B30CustomGiftRequestEvents" (request_id, created_at DESC);
...
async function runSchemaMigration() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock($1)', [2002296001]);
...
async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = runSchemaMigration().catch(err => {
      schemaReady = null;
      throw err;
    });
  }
